...
<details>
<summary><strong>📌 Data Fetching Patterns – Parallel Data Fetching</strong></summary>

In Parallel data fetching requests in a route are eagerly initiated and will load data at the same time. This reduces the total time it takes to load data.

The key to parallel data fetchinf is initiating the requests before awaiting any of them

Is useful when you have multiple independent pieces of data that you need to fetch instead of fetching one after another you can fetch them all at once and reduce the total loading time


```tsx
//data-fetching-demo/src/app/user-parallel/[id]/page.tsx
...
//an awaited request inside the component will block any requests
//below it. This is why we initiated both requests before awaiting any of them
